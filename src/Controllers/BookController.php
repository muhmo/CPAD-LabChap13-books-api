...
201)
 ...
 if (!$isOwner && !$isAdmin){
    ...
    $stmt = $this->books->pdo()->prepare(
        "INSERT INTO audit_log (actor_id, action, target, ip_address)
         VALUES (:actor, 'forbidden_update', CONCAT('books/', :bookId), :ip)"
    );
    $stmt->execute([
        'actor' => (int)($auth['sub'] ?? 0),
        'bookId' => $id,
        'ip' => $ip
    ]);
...
public function delete(Request $req, Response $s, array $a): Response {
...
if (($auth['role'] ?? 'member') !== 'admin') {
 ...
 private function validate(array $body, bool $requireAll): array {
    ...
    // ✅ Always return an array (empty if valid)
    return $errors;
}
